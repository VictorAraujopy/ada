...
juizes = {}
for arq in sorted((AQUI / "resultados").glob("julgamento_cego_*.md")):
    juiz = arq.stem.removeprefix("julgamento_cego_")
...
if juizes:
    ...
    for lado in ("A", "B"):
        ...
        placar[lado]["raciocinio"] = {"acertos": round(sum(j["votos"][lado] for j in juizes.values()) / n, 1),
                                      ...
                                      "pct": round(pct)}
    if len(juizes) == 2:
        ev, ef = (j["escolhas"] for j in juizes.values())
        comum = set(ev) & set(ef)
        iguais = sum(1 for p in comum if ev[p] == ef[p])
        print(f"concordância entre os juízes ({' x '.join(juizes)}): {iguais}/{len(comum)}")
else:
...
NOMES = {"matematica": "Matemática do dia a dia", "matematica_dificil": "Matemática em várias etapas",
         "logica": "Lógica", "uso_tool": "Uso correto de ferramentas",
         "uso_tool_dificil": "Ferramenta com pedido indireto", "armadilha_tool": "Resistência a tool espúria",
         "armadilha_dificil": "Armadilha difícil de tool", "factual": "Precisão factual (TWD)",
         "ambiguo": "Pede contexto quando falta", "raciocinio": "Raciocínio (julgado)"}
ORDEM = [c for c in NOMES if c in placar["A"]]

BG, INK, DIM, GRID = "#0a0908", "#ece7de", "#847d72", "#2a2724"
COR_A, COR_B = "#56504a", "#b3171d"   # ADA antiga cinza, ADA nova sangue
...
s = [f'<svg xmlns="http://www.w3.org/2000/svg" width="{W}" height="{H}" font-family="Georgia, serif">',
     ...
     f'<text x="40" y="56" fill="{INK}" font-size="30" letter-spacing="6">ADA — 9B vs 27B</text>',
     f'<text x="40" y="84" fill="{DIM}" font-size="13" font-family="Helvetica" letter-spacing="1">'
     f'avaliação interna · {N_PERGUNTAS} perguntas fixas · mesmo dataset (v12), LoRA em cada base · 100% local</text>',
     f'<rect x="40" y="104" width="14" height="14" fill="{COR_A}"/>'
     f'<text x="62" y="116" fill="{DIM}" font-size="13" font-family="Helvetica">v12 · Qwen3.5-9B 4-bit · português</text>',
     f'<rect x="340" y="104" width="14" height="14" fill="{COR_B}"/>'
     f'<text x="362" y="116" fill="{INK}" font-size="13" font-family="Helvetica">v12_1_en · Qwen3.8-27B 3-bit · inglês</text>']
...
s.append(f'<text x="40" y="{H - 28}" fill="{DIM}" font-size="11.5" font-family="Helvetica">'
         f'Categorias objetivas pontuadas por script contra gabarito. Raciocínio julgado por IA — não é cego: '
         f'cada modelo responde no idioma do seu treino.</text>')
...
(AQUI / "grafico_benchmark.svg").write_text("\n".join(s), encoding="utf-8")  # cópia versionada (resultados/ é ignorado)
